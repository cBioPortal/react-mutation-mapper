...
export default class CivicCard extends React.Component<ICivicCardProps, {}> {
        ...
        if (variantMap) {
            if (_.isEmpty(variantMap)) {
                ...
                for (let name in variantMap) {
                    ...
                    list.push(this.variantItem(name, variant.url, variant.name, entryTypes, variant.description));
                }
            }
        } else {
            list.push(this.variantItem("_naVariant_"));
     ...
     * @returns {any}
     */
    public variantItem(key: string, url?: string, name?: string, entryTypes?: string, description?: string) {
        let result;
